...
from sklearn.model_selection import cross_val_score as cvs # k-fold Crossvalidation zur Validierung des Trainingssets
from sklearn.preprocessing import StandardScaler,RobustScaler # Standardisierung der Daten 
from sklearn.svm import SVC # SVC = support vector classifier 
from sklearn import svm # svm = support vector machine
from sklearn.metrics import accuracy_score # Validierung des Algorithmus anhand des Testsets
from sklearn.metrics import confusion_matrix # Matrix Zusammenfassung der Fehler 1. und 2. Ordnung 
from sklearn.metrics import classification_report # Präzision, recall, f1-score etc.
from sklearn.metrics import roc_curve #ROC = Receiver Operating Characteristics Curve Visualisierung der Performance des Classifiers 
...
def display_Click(L_value,a_value,b_value,Submit):
    ...
    if 'sub' in changed_id:            
        df = pd.read_csv(r"C:\Users\mhens\Documents\Blanc_de_Noir_check_training_data.csv", delimiter=';')
        #data curation
        y_id=df['target_id']
        y=df['target']
        ...
        if pred_new_wine == 1:
            return f'this wine is most liekely a Blanc de Noir with an accuracy of 95%' 
        else:
            return f'this wine is most likely NOT a Blanc de Noir with an accuracy of 95%'
    else:
        return None
...
def displayClick(L_value,a_value,b_value,submit):
    ...
    if 'sub' in changed_id:
        ...
        model =svm.SVC(kernel='rbf', gamma=0.2, C=48,) 
        clf = model.fit(X, y)
        target_3f=pd.read_csv(r"C:\Users\mhens\Scripts\Machine learning\Blanc de Noir offensive\plotting_reference.csv", delimiter=';')
...
def displayClick(L_value,a_value,b_value,submit):
    changed_id = [p['prop_id'] for p in callback_context.triggered][0]
    if 'sub' in changed_id:
        df = pd.read_csv(r"C:\Users\mhens\Documents\Blanc_de_Noir_check_training_data.csv", delimiter=';') # Pandas DataFrame aus Exceldatei
        y_id=df['target_id']
